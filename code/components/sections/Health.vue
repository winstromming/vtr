  ...
  <n-card title="Health" :bordered="false" size="small">
    <template #header-extra>
      <n-popover trigger="click" placement="top">
        <template #trigger>
          <n-button text strong type="error" title="Damage"> Take Damage </n-button>
        </template>
    ...
    <n-space vertical size="small">
      <n-space :size="5" style="margin-bottom: 3px">
        <n-tooltip trigger="hover" v-for="(amount, index) of character.health.aggravated" :disabled="character.health.lethal > 0 || index + 1 !== character.health.aggravated">
          <template #trigger>
            <n-button :disabled="character.health.lethal > 0 || index + 1 !== character.health.aggravated" text @click="healDamage">
              <n-icon size="20"><SquareAsterisk stroke-width="1.5" /></n-icon>
            </n-button>
          </template>
          Heal (1 week)
        </n-tooltip>
        <n-tooltip trigger="hover" v-for="(amount, index) of character.health.lethal" :disabled="character.health.bashing > 0 || index + 1 !== character.health.lethal">
          <template #trigger>
            <n-button :disabled="character.health.bashing > 0 || index + 1 !== character.health.lethal" text @click="healDamage">
              <n-icon size="20"><SquareX stroke-width="1.5" /></n-icon>
            </n-button>
          </template>
          Heal (2 days)
        </n-tooltip>
        <n-tooltip trigger="hover" v-for="(amount, index) of character.health.bashing" :disabled="index + 1 !== character.health.bashing">
          <template #trigger>
      ...
      <n-input-number class="text-right" v-model:value="character.traits.Willpower" placeholder="0" min="0" :max="maxWillpower">
        <template #prefix>
      ...
      <n-input-number class="text-right" v-model:value="character.traits.Vitae" placeholder="0" min="0" :max="maxVitae">
        <template #prefix>
          <n-text strong>Vitae</n-text>
        </template>
        <template #suffix>
          <n-text>/ {{ maxVitae }}</n-text>
        </template>
      </n-input-number>
    </n-space>
  </n-card>
...
const maxVitae = computed(() => {
  switch (character.traits.BloodPotency) {
    case 0:
      return 0 + character.attributes.physical.Stamina.dots + resilience.value;
    case 1:
      return 10;
    case 2:
      return 11;
